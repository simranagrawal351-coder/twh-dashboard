...
export async function GET(){
 ...
 try {const result=await recordsDb().prepare('SELECT id,payload,version,updated_at FROM records').all<{id:string;payload:string;version:number;updated_at:string}>();
 const merged=new Map(seed.map(r=>[r.id,r]));for(const r of result.results)merged.set(r.id,{...JSON.parse(r.payload),version:r.version,updatedAt:r.updated_at});return json({records:[...merged.values()]});
...
export async function PUT(req:Request){
 ...
 const existing=await db.prepare('SELECT payload FROM records WHERE id = ?').bind(entry.id).first<{payload:string}>();const original=existing?JSON.parse(existing.payload):seed.find(r=>r.id===entry.id);if(original&&original.section!==entry.section)return json({error:'Record type cannot be changed.'},400);
