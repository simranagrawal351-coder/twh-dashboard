...
export const completeStatuses = ['Approved','Done','Published','Delivered','Brief confirmed','Reference','Archived','Deleted','Cancelled','Completed','Superseded','Rejected'];
...
export function attentionRecords(records:Entry[]){
 const visible=records.filter(r=>r.section!=='private'&&!['Archived','Deleted','Cancelled'].includes(r.status));
